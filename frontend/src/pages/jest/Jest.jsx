  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await Axios.post(
        process.env.REACT_APP_NODE_JEST_API_BASE_URL + "/api",
        input
      );
  ...
  return (
    <div className="flex flex-col items-center justify-center pt-32 bg-dark min-h-screen h-full w-full" id="colo">
      <button
        ...
        onClick={() => navigate("/jest-history")}
      >
        <div style={{ display: "flex", justifyContent: "center" }}>
          <img src={list} alt="list" width={36} />
        </div>
      </button>
      {!result && <h2 className="font-bold text-light">Jest</h2>}
      <br />
      {!result && (
        <form
